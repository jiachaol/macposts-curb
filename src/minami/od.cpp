...
MNM_Origin::MNM_Origin(TInt ID, TInt max_interval, TFlt flow_scalar, TInt frequency)
{
  ...
  m_demand = std::unordered_map<MNM_Destination*, TFlt*>();  // destination node, time-varying demand list
}
                                    ...
                                    TInt assign_interval, TFlt adaptive_ratio)
{
  ...
  for (auto _demand_it = m_demand.begin(); _demand_it != m_demand.end(); _demand_it++) {
    ...
    for (int i = 0; i < _veh_to_release; ++i) {
      ...
      m_origin_node -> m_in_veh_queue.push_back(_veh); //Adds a new element at the end of the vector, after its current last element.
    }
  }
  return 0;
}
