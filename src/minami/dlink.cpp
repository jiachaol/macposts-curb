...
int MNM_Dlink_Ctm::evolve(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
TFlt MNM_Dlink_Ctm::get_link_capacity(){
    return 0;
}

int MNM_Dlink_Ctm::clear_incoming_array(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
TFlt MNM_Dlink_Pq::get_link_capacity(){
    return m_lane_flow_cap * TFlt(m_number_of_lane) * m_unit_time;
}

int MNM_Dlink_Pq::clear_incoming_array(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
int MNM_Dlink_Pq::evolve(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
TFlt MNM_Dlink_Lq::get_link_capacity(){
    return 0;
}

int MNM_Dlink_Lq::clear_incoming_array(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
int MNM_Dlink_Lq::evolve(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
    // printf("1\n");
        ...
        if (m_recorder[i].first >= time) {
            return m_recorder[i - 1].second
                   + (m_recorder[i].second - m_recorder[i - 1].second) / (m_recorder[i].first - m_recorder[i - 1].first)
                     * (time - m_recorder[i - 1].first); // interpolation
        }
    }
...
int MNM_Dlink_Ltm::clear_incoming_array(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
...
int MNM_Dlink_Ltm::evolve(TInt timestamp, TFlt ratio_lane_closure = 1.0) {
