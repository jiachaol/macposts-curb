...
int MNM_Routing_Adaptive::update_routing(TInt timestamp)
{
  ...
  if ((timestamp) % m_routing_freq  == 0 || timestamp == 0) {
    ...
    // printf(" node 5 shortest path = %s\n", _shortest_path_tree[5]);
  }
...
// Jiachao
int MNM_Routing_Biclass_Hybrid::init_routing_curb(Path_Table *path_table_car, Path_Table *path_table_truck, Path_Table *path_table_rh)
{
  return 0;
}
