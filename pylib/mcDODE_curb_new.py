...
class MCDODE():
    ...
    def add_data(self, data_dict):
        ...
        # TODO one_data_dict['truck_curb_agg_N']
    ...
    # the DAR matrix in this function is mapping path flow to link flow (non-stop link flow)
    ...
    # DAR matrix mapping path flow to curb arrival and departure
    ...
    # observation should add both car and non-stop RH
    def _compute_count_loss_grad_on_car_link_flow(self, dta, one_data_dict):
        link_flow_array = one_data_dict['car_link_flow'] # both car + RH

        x_e_car = dta.get_link_car_inflow(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                      np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq).flatten(order='F')

        if self.config['car_count_agg']:
            x_e_car = one_data_dict['car_count_agg_L'].dot(x_e_car)

        x_e_rh = dta.get_link_rh_inflow(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                      np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq).flatten(order='F')

        if self.config['rh_count_agg']:
            x_e_rh = one_data_dict['rh_count_agg_L'].dot(x_e_rh)
        ...
        if self.config['car_count_agg']:
            grad_car = one_data_dict['car_count_agg_L'].T.dot(grad)
        
        if self.config['rh_count_agg']:
            grad_rh = one_data_dict['rh_count_agg_L'].T.dot(grad)
    ...
    def _get_loss(self, one_data_dict, dta):
        ...
        if self.config['compute_car_link_flow_loss']:
            x_e_car = dta.get_link_car_inflow(assign_intervals, assign_intervals + self.ass_freq).flatten(order='F')
            if self.config['car_count_agg']:
                x_e_car = one_data_dict['car_count_agg_L'].dot(x_e_car)
        ...
        if self.config['compute_truck_link_flow_loss']:
    ...
    def compute_path_flow_grad_and_loss(self, one_data_dict, f_car, f_truck, f_rh, counter=0):
        ...
        if self.config['use_car_link_flow']:
            ...
            grad_car, grad_rh, x_e_car = self._compute_count_loss_grad_on_car_link_flow(dta, one_data_dict)
        ...
        #     grad = self._compute_count_loss_grad_on_rh_link_flow(dta, one_data_dict)
