...
class MCDODE():
    def __init__(self, nb, config):
        ...
        self.num_path_curb_rh = nb.config.config_dict['FIXED']['num_path_curb_rh'] # new added
        self.num_loading_interval = self.num_assign_interval * self.ass_freq
        self.data_dict = dict()
        self.num_data = self.config['num_data']

        self.observed_links_count = self.config['observed_links_count']
        self.observed_links_tt = self.config['observed_links_tt']
        self.observed_links_curb = self.config['observed_links_curb']

        self.paths_list = self.config['paths_list']
        self.paths_list_cc = self.config['paths_list_cc']
        self.paths_list_cc_rh = self.config['paths_list_cc_curb']
        ...
        assert (len(self.paths_list_cc_rh) == self.num_path_curb_rh)
    ...
    def get_dar_count(self, dta, f_car, f_truck, f_rh):
        car_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_count), self.num_assign_interval * len(self.paths_list)))
        truck_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_count), self.num_assign_interval * len(self.paths_list_cc)))
        rh_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_count), self.num_assign_interval * len(self.paths_list_cc_rh)))
        ...
        if self.config['use_car_link_flow'] or self.config['use_car_link_tt']:
            ...
            car_dar = self._massage_raw_dar(raw_car_dar, self.ass_freq, f_car, self.num_assign_interval, self.paths_list, self.observed_links_count)
        ...
        if self.config['use_truck_link_flow'] or self.config['use_truck_link_tt']:
            ...
            raw_truck_dar = dta.get_truck_dar_matrix_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            # num_assign_interval * num_e_link, num_assign_interval * num_e_path
            truck_dar = self._massage_raw_dar(raw_truck_dar, self.ass_freq, f_truck, self.num_assign_interval, self.paths_list_cc, self.observed_links_count)
        ...
        if self.config['use_rh_link_flow'] or self.config['use_rh_link_tt']:
            ...
            raw_rh_dar = dta.get_rh_dar_matrix_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            # num_assign_interval * num_e_link, num_assign_interval * num_e_path
            rh_dar = self._massage_raw_dar(raw_rh_dar, self.ass_freq, f_rh, self.num_assign_interval, self.paths_list_cc_rh, self.observed_links_count)
        
        return (car_dar, truck_dar, rh_dar)

    # TODO DAR matrix for TT observed links
    def get_dar_tt(self, dta, f_car, f_truck, f_rh):
        car_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), self.num_assign_interval * len(self.paths_list)))
        truck_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), self.num_assign_interval * len(self.paths_list_cc)))
        rh_dar = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), self.num_assign_interval * len(self.paths_list_cc_rh)))
        ...
        if self.config['use_car_link_flow'] or self.config['use_car_link_tt']:
            ...
            # print("raw car dar", raw_car_dar)
            # num_assign_interval * num_e_link, num_assign_interval * num_e_path
            car_dar = self._massage_raw_dar(raw_car_dar, self.ass_freq, f_car, self.num_assign_interval, self.paths_list, self.observed_links_tt)
        ...
        if self.config['use_truck_link_flow'] or self.config['use_truck_link_tt']:
            ...
            raw_truck_dar = dta.get_truck_dar_matrix_tt(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            # num_assign_interval * num_e_link, num_assign_interval * num_e_path
            truck_dar = self._massage_raw_dar(raw_truck_dar, self.ass_freq, f_truck, self.num_assign_interval, self.paths_list_cc, self.observed_links_tt)
        ...
        if self.config['use_rh_link_flow'] or self.config['use_rh_link_tt']:
            ...
            raw_rh_dar = dta.get_rh_dar_matrix_tt(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            # num_assign_interval * num_e_link, num_assign_interval * num_e_path
            rh_dar = self._massage_raw_dar(raw_rh_dar, self.ass_freq, f_rh, self.num_assign_interval, self.paths_list_cc_rh, self.observed_links_tt)
        
        return (car_dar, truck_dar, rh_dar)

    # DAR matrix for curbside arrival and departure
    def get_dar_arrival(self, dta, f_truck, f_rh):
        truck_dar_arrival = csr_matrix((self.num_assign_interval * len(self.observed_links_curb), self.num_assign_interval * len(self.paths_list_cc)))
        rh_dar_arrival = csr_matrix((self.num_assign_interval * len(self.observed_links_curb), self.num_assign_interval * len(self.paths_list_cc_rh)))
        ...
        if self.config['use_truck_link_curb_arrival']:
            ...
            raw_truck_dar_arrival = dta.get_truck_dar_arrival_matrix_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            truck_dar_arrival = self._massage_raw_dar(raw_truck_dar_arrival, self.ass_freq, f_truck, self.num_assign_interval, self.paths_list_cc, self.observed_links_curb)
        ...
        if self.config['use_rh_link_curb_arrival']:
            ...
            raw_rh_dar_arrival = dta.get_rh_dar_arrival_matrix_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            rh_dar_arrival = self._massage_raw_dar(raw_rh_dar_arrival, self.ass_freq, f_rh, self.num_assign_interval, self.paths_list_cc_rh, self.observed_links_curb)

        return (truck_dar_arrival, rh_dar_arrival)
    
    def get_dar_departure(self, dta, f_truck, f_rh):
        truck_dar_departure = csr_matrix((self.num_assign_interval * len(self.observed_links_curb), self.num_assign_interval * len(self.paths_list_cc)))
        rh_dar_departure = csr_matrix((self.num_assign_interval * len(self.observed_links_curb), self.num_assign_interval * len(self.paths_list_cc_rh)))
        ...
        if self.config['use_truck_link_curb_departure']:
            ...
            raw_truck_dar_departure = dta.get_truck_dar_departure_matrix_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            truck_dar_departure = self._massage_raw_dar(raw_truck_dar_departure, self.ass_freq, f_truck, self.num_assign_interval, self.paths_list_cc, self.observed_links_curb)
        ...
        if self.config['use_rh_link_curb_departure']:
            ...
            raw_rh_dar_departure = dta.get_rh_dar_departure_matrix(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                                    np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq)
            rh_dar_departure = self._massage_raw_dar(raw_rh_dar_departure, self.ass_freq, f_rh, self.num_assign_interval, self.paths_list_cc_rh, self.observed_links_curb)
    ...
    def get_ltg(self, dta):
        car_ltg_matrix = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), 
                                             self.num_assign_interval * len(self.paths_list)))
     
        truck_ltg_matrix = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), 
                                               self.num_assign_interval * len(self.paths_list_cc)))
        
        rh_ltg_matrix = csr_matrix((self.num_assign_interval * len(self.observed_links_tt), 
                                               self.num_assign_interval * len(self.paths_list_cc_rh)))
    ...
    def init_demand_vector_truck(self, num_assign_interval, num_col_nonstop, num_col_stop, scale_nonstop, scale_stop):
        ...
        d_stop = np.random.rand(num_assign_interval * num_col_stop) * scale_stop
    ...
    def init_path_flow(self, car_scale, truck_scale, rh_scale):
        ...
        f_rh = self.init_demand_vector(self.num_assign_interval, self.num_path_curb_rh, rh_scale)
    ...
    def _compute_link_tt_grad_on_path_flow_car(self, dta):
        ...
        ltg = self._massage_raw_ltg(raw_ltg, self.ass_freq, num_assign_intervals, self.paths_list, self.observed_links_tt)
        return ltg
    ...
    def _compute_link_tt_grad_on_path_flow_truck(self, dta):
        ...
        ltg = self._massage_raw_ltg(raw_ltg, self.ass_freq, num_assign_intervals, self.paths_list_cc, self.observed_links_tt)
        return ltg
    ...
    def _compute_link_tt_grad_on_path_flow_rh(self, dta):
        ...
        ltg = self._massage_raw_ltg(raw_ltg, self.ass_freq, num_assign_intervals, self.paths_list_cc_rh, self.observed_links_tt)
    ...
    def _run_simulation_curb(self, f_car, f_truck, f_rh, counter, show_loading = True):
        ...
        # register paths
        a.register_paths_car(self.paths_list) # m_path_vec, m_path_set

        # m_path_vec should be different from m_path_vec_curb !!!!
        a.register_paths_truck(self.paths_list_cc) # m_path_vec_truck, m_path_set_truck

        a.register_paths_rh(self.paths_list_cc_rh) # m_path_vec_rh, m_path_set_rh

        # install cc and cc_tree on registered links separately
        a.install_cc_separate()
        a.install_cc_tree_separate()
    ...
    def _compute_count_loss_grad_on_rh_link_flow(self, dta, one_data_dict):
        ...
        x_e = dta.get_link_rh_inflow_sep(np.arange(0, self.num_loading_interval, self.ass_freq), 
                                      np.arange(0, self.num_loading_interval, self.ass_freq) + self.ass_freq).flatten(order='F')
        # print("x_e", x_e, link_flow_array)
        if self.config['rh_count_agg']:
    ...
    # TODO new function for getting car and rh grad together
    ...
    def _get_loss(self, one_data_dict, dta):
        ...
        if self.config['use_car_link_tt'] or self.config['compute_car_link_tt_loss']:
            ...
            loss = self.config['link_car_tt_weight'] * np.linalg.norm(np.nan_to_num(x_tt_e - x_tt_r))
        ...
        if self.config['use_truck_link_tt'] or self.config['compute_truck_link_tt_loss']:
            ...
            loss = self.config['link_truck_tt_weight'] * np.linalg.norm(np.nan_to_num(x_tt_e - x_tt_r))
            loss_dict['truck_tt_loss'] = loss

        # add loss for truck and RH curb arrival and departures
        if self.config['use_truck_curb_count'] or self.config['compute_truck_curb_count_loss']:
    ...
    def compute_path_flow_grad_and_loss(self, one_data_dict, f_car, f_truck, f_rh, counter=0):
        ...
        # use _compute_count_loss_grad_on_car_rh_link_flow  --  0614  -- Jiachao
        ...
        if self.config['use_rh_link_flow']:
            rh_grad += self.config['link_rh_flow_weight'] * grad_rh
        ...
        # truck_curb_arr_weight, truck_curb_dep_weight, rh_curb_arr_weight, rh_curb_dep_weight
    ...
    def estimate_path_flow_pytorch_curb(self, 
        ...
        if normalized_by_scale:
            ...
            f_rh_tensor = torch.from_numpy(f_rh / np.maximum(car_init_scale, 1e-6))
        else:
